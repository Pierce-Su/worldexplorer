...
Pipeline stages:
    Stage 1: Scaffold Generation - Creates panoramic images from text (or from a starting image)
...
For each sample, the pipeline runs once per available variant: if the sample has a photorealistic
image, a scene is generated with that as 000.png and written under output_base/photorealistic/index_xxxx/;
if it has a stylized image, another scene is generated under output_base/stylized/index_xxxx/.

Output structure (when samples have photorealistic and/or stylized images):
    output_base/
        photorealistic/
            index_0000/
                scaffold/
...
def process_scaffold_stage(
    ...
    """Process Stage 1: Scaffold Generation. When input_image_path is set (e.g. for a variant), use it as 000.png."""
    index = sample["index"]
    ...
    # Starting image for scaffold (000.png / imag_0). If not passed in (variant run), resolve from metadata.
    if input_image_path is None:
        scaffold_cfg = scaffold_config
        if scaffold_cfg.get("input_image"):
            path_val = scaffold_cfg["input_image"]
            input_image_path = dataset_dir / path_val if not os.path.isabs(path_val) else Path(path_val)
        elif scaffold_cfg.get("imag_0"):
            path_val = scaffold_cfg["imag_0"]
            input_image_path = dataset_dir / path_val if not os.path.isabs(path_val) else Path(path_val)
        else:
            input_image_path = None
        if input_image_path is not None and not input_image_path.exists():
            print(f"  WARNING: Scaffold starting image not found: {input_image_path}")
            input_image_path = None
    if input_image_path is not None:
        print(f"  Using starting image as 000.png (imag_0): {input_image_path}")
...
def process_sample(
    ...
    """Process a single sample from metadata. When input_image_path is set, use it as 000.png for this run (e.g. one variant)."""
    index = sample["index"]
    ...
    if only_stages is None or 1 in only_stages:
        ...
        # Check if scaffold already exists (e.g. when skipping stage 1)
...
Examples:
  ...
  python run_batch_pipeline.py --dataset_dir data/curated_set \\
    ...
    def get_variants_for_sample(s: dict) -> List[tuple]:
        out = []
        if "photorealistic" in s:
            path = s["photorealistic"].get("original_path")
            if path:
                p = dataset_dir / path
                if p.exists():
                    out.append(("photorealistic", p))
        if "stylized" in s:
            path = s["stylized"].get("original_path")
            if path:
                p = dataset_dir / path
                if p.exists():
                    out.append(("stylized", p))
        return out

    total_jobs_pre = sum(len(get_variants_for_sample(s)) for s in samples)
    ...
    print(f"Output base: {output_base} (photorealistic/ and stylized/ subdirs)")
    print(f"Total samples: {len(samples)} → {total_jobs_pre} jobs (one per variant per sample)")
    if args.only_stages:
    ...
    # Process each sample × variant (output under output_base/{photorealistic|stylized}/index_xxxx)
    all_results = []
    start_time = time.time()
    total_jobs = sum(len(get_variants_for_sample(s)) for s in samples)
    if total_jobs == 0:
        print("ERROR: No jobs to run. Each sample must have at least one of photorealistic.original_path or stylized.original_path pointing to an existing file.")
        sys.exit(1)
    job_num = 0

    for i, sample in enumerate(samples, 1):
        index = sample["index"]
        variants = get_variants_for_sample(sample)
        if not variants:
            print(f"\nSkipping sample {index}: no photorealistic or stylized image path found")
            continue
        ...
        for variant_name, variant_image_path in variants:
            ...
            print(f"\n{'='*80}")
            print(f"Processing job {job_num}/{total_jobs}: index {index} [{variant_name}]")
            print(f"{'='*80}")
            ...
            try:
                ...
                if result["status"] == "success":
                    print(f"✓ Successfully processed index {index} [{variant_name}]")
                    print(f"  Stages completed: {result['stages_completed']}")
                else:
                    print(f"✗ Failed index {index} [{variant_name}]")
                    if not args.continue_on_error:
                        print("Stopping due to error (use --continue_on_error to continue)")
                        break

            except Exception as e:
                print(f"ERROR processing index {index} [{variant_name}]: {e}")
                import traceback
                traceback.print_exc()
                all_results.append({
                    "index": index,
    ...
    print(f"{'='*80}")
    print(f"Total jobs (variant runs): {len(all_results)}")
