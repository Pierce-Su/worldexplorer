...
# Constants.
# Default WORK_DIR - can be overridden by command line parameter
WORK_DIR = "work_dirs/backwards"
...
def parse_task(
    ...
    if task == "img2trajvid_s-prob":
        ...
        if task == "img2img":
            ...
            # anchor_c2ws, anchor_Ks, anchor_indices all correspond, whereby anchors are just a set of tracectory locations sampled from all targets
...
def main(
    ...
    work_dir=None,  # New parameter to override WORK_DIR
    global_samples_file=None,  # New parameter
    translation_scaling_factor=3,
    enable_collision_detection=True,
    **overwrite_options,
):
    def get_trajectory_type_and_id(file_path):
        # Get the last part of the path
        last_folder = os.path.basename(file_path)
        # Split by underscore
        parts = last_folder.split('_')
        # The trajectory type is the middle part (index 1)
        # The initial image ID is the first part (index 0)
        if len(parts) >= 3 and parts[-1] == 'wrapper':
            return parts[1], parts[0]  # Return as tuple: (traj_type, initial_image_id)
        return None, None  # Return None for both if the format doesn't match
    
    traj_type, initial_image_id = get_trajectory_type_and_id(data_path)
    
    global WORK_DIR  # Access the global variable
    
    if work_dir is not None:
        WORK_DIR = work_dir  # Override the global variable if provided
    ...
    for scene in tqdm(scenes):
        ...
        img_count = min(len(img_paths), num_targets)
        if len(img_paths) != len(c2ws):
            input_img_paths = sorted(
                glob.glob(osp.join(save_path_scene, "input", "*.png"))
            )
            # Use min to avoid assertion errors with truncated frames
            input_count = num_inputs
            img_count = min(len(img_paths), num_targets)

            # Modify c2ws and Ks to match the actual number of frames
            if img_count < num_targets:
                # Only use c2ws and Ks for frames we actually have
                c2ws = c2ws[:num_inputs + img_count]
                Ks = Ks[:num_inputs + img_count]
            
            assert len(input_img_paths) == input_count, f"Expected {num_inputs} input frames, found {len(input_img_paths)}"
            assert c2ws.shape[0] == input_count + img_count, f"Expected {input_count + img_count} camera transforms, found {c2ws.shape[0]}"
            
            target_indices = [i for i in range(input_count + img_count) if i not in input_indices[:input_count]]
            
            # Adjust how we construct img_paths to handle potential truncation
            old_img_paths = img_paths.copy()
            img_paths = []
            for i in range(input_count + img_count):
                if i < input_count and i in input_indices[:input_count]:
                    img_paths.append(input_img_paths[input_indices[:input_count].index(i)])
                elif i - input_count < len(target_indices) and i - input_count < len(old_img_paths):
                    img_paths.append(old_img_paths[target_indices.index(i)])
                else:
                    # Skip this frame if we don't have corresponding img_path
                    continue
            # input_img_paths = sorted(
            #     glob.glob(osp.join(save_path_scene, "input", "*.png"))
            # )
